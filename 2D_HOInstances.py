...
def run_dc(dc_train_features, dc_test_features, train_labels, test_labels, class_length):
    ...
    dc_train_features = np.reshape(dc_train_features, (dc_train_features.shape[0],dc_train_features.shape[2], 40, 30))
    print(dc_train_features.shape)
    ...
    dc_test_features = np.reshape(dc_test_features, (dc_test_features.shape[0],dc_test_features.shape[2], 40, 30))
    print(dc_test_features.shape)
...
def run_ac(ac_train_features, ac_test_features, train_labels, test_labels, class_length):
    ...
    ac_train_features = np.reshape(ac_train_features, (ac_train_features.shape[0], ac_train_features.shape[1],ac_train_features.shape[2]))
    print(ac_train_features.shape)
    ...
    ac_test_features = np.reshape(ac_test_features, (ac_test_features.shape[0], ac_test_features.shape[1], ac_test_features.shape[2]))
    print(ac_test_features.shape)
...
def get_hold_out_instances(features, labels):
    indices = np.random.choice(len(features), int(len(features)/3), False)
    test_features = [u for indd,u in enumerate(features) if indd in indices]
    train_features = [u for indd,u in enumerate(features) if indd not in indices]
    test_labels = [u for indd,u in enumerate(labels) if indd in indices]
    train_labels = [u for indd,u in enumerate(labels) if indd not in indices]
...
def run():
    ...
    train_features, test_features, train_labels, test_labels = get_hold_out_instances(features, labels)
    features = None
    labels = None
