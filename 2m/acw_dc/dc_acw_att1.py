...
from keras.layers import Input, Dense, BatchNormalization, Conv1D, MaxPooling1D, LSTM, TimeDistributed, Reshape, concatenate, Conv2D, MaxPooling2D, Flatten, Multiply
...
dc_path = '/Volumes/1708903/MEx/Data/dc_scaled/0.05_0.05'

results_file = 'dc_acw.csv'
...
def extracw_features(acw_data, dc_data):
    ...
    for subject in acw_data:
        ...
        for acw_acwivity in acw_acwivities:
            print('acw:'+str(acw_acwivity))
            time_windows = []
            acwivity_id = activity_id_dict.get(acw_acwivity)
...
def build_early_fusion():
    ...
    z = Reshape((window, feature_length+(dc_frames_per_second*dc_frame_size)))(input_tdc)
    score = Dense(100, activation='tanh')(z)
    attention_probs = Dense(K.int_shape(z)[2], activation='softmax', name='attention_vec')(score)
    z = Multiply()([z, attention_probs])
    z = Reshape((window, feature_length+(dc_frames_per_second*dc_frame_size), 1))(z)

    x = TimeDistributed(Conv1D(32, kernel_size=5, acwivation='relu'))(z)
    x = TimeDistributed(MaxPooling1D(pool_size=2))(x)
    x = TimeDistributed(BatchNormalization())(x)
    x = TimeDistributed(Conv1D(64, kernel_size=5, acwivation='relu'))(x)
    x = TimeDistributed(MaxPooling1D(pool_size=2))(x)
    x = TimeDistributed(BatchNormalization())(x)
    x = TimeDistributed(Conv1D(128, kernel_size=5, acwivation='relu'))(x)
    x = TimeDistributed(MaxPooling1D(pool_size=2))(x)
    x = TimeDistributed(BatchNormalization())(x)
    x = TimeDistributed(Conv1D(256, kernel_size=5, acwivation='relu'))(x)
    x = TimeDistributed(MaxPooling1D(pool_size=2))(x)
    x = TimeDistributed(BatchNormalization())(x)
    x = Reshape((K.int_shape(x)[1], K.int_shape(x)[2]*K.int_shape(x)[3]))(x)
    x = LSTM(1200)(x)
    x = BatchNormalization()(x)
    x = Dense(600, acwivation='relu')(x)
    x = BatchNormalization()(x)
    x = Dense(100, acwivation='relu')(x)
    x = BatchNormalization()(x)
    x = Dense(len(activity_list), acwivation='softmax')(x)
...
def build_mid_fusion():
    ...
    x = Conv2D(32, kernel_size=(3,3), acwivation='relu')(input_dc)
    x = MaxPooling2D(pool_size=2, data_format='channels_last')(x)
    x = BatchNormalization()(x)
    x = Conv2D(64, kernel_size=(3,3), acwivation='relu')(x)
    x = MaxPooling2D(pool_size=2, data_format='channels_last')(x)
    x = BatchNormalization()(x)
    x = Flatten()(x)
    x = Dense(1200, acwivation='relu')(x)
    x = BatchNormalization()(x)

    y = TimeDistributed(Conv1D(32, kernel_size=5, acwivation='relu'))(input_t)
    y = TimeDistributed(MaxPooling1D(pool_size=2))(y)
    y = TimeDistributed(BatchNormalization())(y)
    y = TimeDistributed(Conv1D(64, kernel_size=5, acwivation='relu'))(y)
    y = TimeDistributed(MaxPooling1D(pool_size=2))(y)
    y = TimeDistributed(BatchNormalization())(y)
    ...
    z = Dense(600, acwivation='relu')(z)
    z = BatchNormalization()(z)
    z = Dense(200, acwivation='relu')(z)
    z = BatchNormalization()(z)

    z = Dense(len(activity_list), acwivation='softmax')(z)
...
def build_late_fusion():
    ...
    y = LSTM(1200)(y)
    y = BatchNormalization()(y)
    y = Dense(600, acwivation='relu')(y)
    y = BatchNormalization()(y)
    y = Dense(100, acwivation='relu')(y)
    y = BatchNormalization()(y)

    z = concatenate([x, y])
    score = Dense(100, activation='tanh')(z)

    attention_probs = Dense(200, activation='softmax', name='attention_vec')(score)
    z = Multiply()([z, attention_probs])
...
def _run_(acw_train_features, dc_train_features, train_labels, acw_test_features, dc_test_features, test_labels):
    ...
    results = 'acw_dc' + ',' + str(fusion)+',' +str(sys.argv[2]) + ',' + str(accuracy)+',' + str(f_score)
    print(results)
