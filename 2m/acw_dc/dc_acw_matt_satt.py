...
class SensorAttentionLayer(Layer):
    def __init__(self, dims, **kwargs):
        super(SensorAttentionLayer, self).__init__(**kwargs)
        self.dims = dims
        self.W = tf.keras.layers.Dense(self.dims)
        self.V = tf.keras.layers.Dense(1)

    def build(self, input_shape):
        super(SensorAttentionLayer, self).build(input_shape)
    ...
    def compute_output_shape(self, input_shape):
        ...
        return input_shapes[0], self.dims
...
def build_early_fusion():
    ...
    input_tdc = EarlyAttentionLayer(feature_length+(dc_frames_per_second*dc_frame_size))([input_t, input_dc])

    x = TimeDistributed(Conv1D(32, kernel_size=5, acwivation='relu'))(input_tdc)
