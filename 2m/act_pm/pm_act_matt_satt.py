...
def split_windows(act_data, pm_data):
    ...
    print(max(act_frames))
    print(min(act_frames))
...
def build_early_fusion():
    ...
    input_tpm = EarlyAttentionLayer(feature_length+(pm_frames_per_second*pm_frame_size))([input_t, input_pm])

    x = TimeDistributed(Conv1D(32, kernel_size=5, activation='relu'))(input_tpm)
    ...
    model = Model(inputs=[input_t, input_pm], outputs=x)
    model.summary()
    return model
