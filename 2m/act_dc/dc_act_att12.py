...
from keras.layers import Layer, Input, Dense, BatchNormalization, Conv1D, MaxPooling1D, LSTM, TimeDistributed
from keras.layers import Reshape, concatenate, Conv2D, MaxPooling2D, Flatten, Multiply
...
dc_path = '/Users/anjanawijekoon/Data/MEx/dc/min/'

results_file = 'dc_act_att12.csv'
...
class CrossSensorLayer(Layer):
    def __init__(self, dims, **kwargs):
        super(CrossSensorLayer, self).__init__(**kwargs)
        self.dims = dims
        self.W = tf.keras.layers.Dense(self.dims)

    def build(self, input_shape):
        super(CrossSensorLayer, self).build(input_shape)

    def call(self, inputs):
        # (, 200)
        z = inputs[1]-inputs[0]
        print('CrossSensorLayer: '+str(K.int_shape(z)))
        ...
        print('CrossSensorLayer: '+str(K.int_shape(score)))
        # (, 200)
        context_vector = tf.reduce_sum(score, axis=1)
        print('CrossSensorLayer: '+str(K.int_shape(context_vector)))
...
class AverageLayer(Layer):
    ...
    def call(self, inputs):
        # (, 200)
        z = inputs[1]+inputs[0]
        print('AverageLayer: '+str(K.int_shape(z)))
        # (, 1, 200)
        context_vector = z / len(inputs)
        print('AverageLayer: '+str(K.int_shape(context_vector)))
...
class AttentionLayer(Layer):
    ...
    def call(self, inputs):
        # (, 200)
        z = tf.concat(inputs, axis=1)
        print('AttentionLayer: '+str(K.int_shape(z)))
        # (, 1, 200)
        z = tf.expand_dims(z, 1)
        print('AttentionLayer: '+str(K.int_shape(z)))
        # (, 1, 200)
        score = tf.nn.tanh(self.W(z))
        print('AttentionLayer: '+str(K.int_shape(score)))
        # (, 1, 200)
        attention_weights = tf.nn.softmax(score)
        print('AttentionLayer: '+str(K.int_shape(attention_weights)))
        # (, 1, 200)
        context_vector = attention_weights * z
        print('AttentionLayer: '+str(K.int_shape(context_vector)))
        # (, 200)
...
def build_mid_fusion():
    ...
    xy = CrossSensorLayer(1200)([x, y])
    yx = CrossSensorLayer(1200)([y, x])
    zz = AverageLayer()([xy, yx])
...
def build_late_fusion():
    ...
    xx = CrossSensorLayer(100)([x, y])
    yy = CrossSensorLayer(100)([y, x])
    zz = AverageLayer()([xx, yy])

    c = concatenate([z, zz])

    c = Dense(len(activity_list), activation='softmax')(c)

    model = Model(inputs=[input_t, input_dc], outputs=c)
    model.summary()
    return model
...
def _run_(act_train_features, dc_train_features, train_labels, act_test_features, dc_test_features, test_labels):
    ...
    results = 'act_dc' + ',' + str(fusion)+',' + str(i)+',' + str(accuracy)+',' + str(f_score)
    print(results)
