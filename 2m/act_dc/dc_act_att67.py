...
class AttentionLayer(Layer):
    def __init__(self, dims,**kwargs):
        ...
        self.b1 = self.add_weight(name='b1', shape=(self.dims,), initializer='zeros', trainable=True)
        self.w2 = self.add_weight(name='w2', shape=(self.dims, self.dims), initializer='glorot_uniform', trainable=True)
        self.b2 = self.add_weight(name='b2', shape=(self.dims,), initializer='zeros', trainable=True)
    ...
    def call(self, inputs):
        ...
        zz = K.bias_add(zz, self.b1, data_format='channels_last')
        print(K.int_shape(zz))
        zz = self.act_tanh(zz)
        print(K.int_shape(zz))
        zz = K.dot(zz, self.w2)
        print(K.int_shape(zz))
        if self.useB:
            zz = K.bias_add(zz, self.b2, data_format='channels_last')
            print(K.int_shape(zz))
        attention_weights = K.softmax(zz)
...
def build_mid_fusion():
    ...
    z = AttentionLayer(2400, True)([x, y])
...
def build_late_fusion():
    ...
    a = AttentionLayer(200, False)([x, y])
