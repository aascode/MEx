...
def build_mid_fusion():
    ...
    x = TimeDistributed(Conv1D(32, kernel_size=5, activation='relu'))(input_dc)
    ...
    x = LSTM(1200, return_sequences=True)(x)
    ...
    y = LSTM(1200, return_sequences=True)(y)
    y = BatchNormalization()(y)

    c = concatenate([x, y])

    c = Bidirectional(LSTM(800, return_sequences=True))(c)
    ...
    z = Dense(len(activity_list), activation='softmax')(c)
...
def build_late_fusion():
    ...
    y = TimeDistributed(BatchNormalization())(y)
    y = Reshape((K.int_shape(y)[1], K.int_shape(y)[2]*K.int_shape(y)[3]))(y)
    y = Bidirectional(LSTM(1200, return_sequences=True))(y)
    y = BatchNormalization()(y)
    ...
    c = Bidirectional(LSTM(80, return_sequences=True))(c)
    c = BatchNormalization()(c)
    c = LSTM(120)(c)
    c = BatchNormalization()(c)
    c = Dense(100, activation='relu')(c)
