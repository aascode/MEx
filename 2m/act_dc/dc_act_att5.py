...
# from AttnSense paper with W1, b1, w1
...
class AttnSenseLayer(Layer):
    def __init__(self, dims, **kwargs):
        super(AttnSenseLayer, self).__init__(**kwargs)
        self.dims = dims

    def build(self, input_shape):
        self.act_tanh = activations.get('tanh')
        # [200, 200]
        self.w1 = self.add_weight(name='w1', shape=(self.dims, self.dims), initializer='glorot_uniform', trainable=True)
        self.w2 = self.add_weight(name='w2', shape=(self.dims, self.dims), initializer='glorot_uniform', trainable=True)
        # [200,]
        self.b = self.add_weight(name='b', shape=(self.dims,), initializer='zeros', trainable=True)
        super(AttnSenseLayer, self).build(input_shape)
    ...
    def call(self, inputs):
        ...
        print(K.int_shape(z))
        zz = K.dot(z, self.w1)
        print(K.int_shape(zz))
        zzz = K.bias_add(zz, self.b, data_format='channels_last')
        print(K.int_shape(zzz))
        score = self.act_tanh(zzz)
        print(K.int_shape(score))
        mu = K.dot(score, self.w2)
        print(K.int_shape(mu))
        attention_weights = K.softmax(mu)
...
def build_late_fusion():
    ...
    a = AttnSenseLayer(200)([x, y])

    z = Dense(len(activity_list), activation='softmax')(a)
