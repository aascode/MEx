...
from keras.layers import Layer, Input, Dense, BatchNormalization, Conv1D, MaxPooling1D, LSTM, Flatten, TimeDistributed, Reshape, concatenate, add, dot, Permute, multiply
...
# fusion by non-parametric self attention for DCT ACW and ACT (2 modalities)
# from QaulityDeepSense paper
...
results_file = '/Volumes/1708903/MEx/results_lopo/act_acw_self_att.csv'
...
class NoParaSelfAttentionLayer(Layer):
    def __init__(self, **kwargs):
        super(NoParaSelfAttentionLayer, self).__init__(**kwargs)

    def build(self, input_shape):
        super(NoParaSelfAttentionLayer, self).build(input_shape)

    def call(self, inputs):
        # (2, 100)
        z = K.concatenate(inputs, axis=1)
        print(K.int_shape(z))

        # (100, 2)
        z_hat = K.permute_dimensions(z, [0, 2, 1])
        print(K.int_shape(z_hat))

        # (2, 2)
        att = K.batch_dot(z, z_hat)
        print(K.int_shape(att))

        # (2)
        att = K.sum(att, axis=1)
        print(K.int_shape(att))
    ...
    def compute_output_shape(self, input_shape):
        ...
        return input_shapes[0][0], len(input_shapes), input_shapes[0][2]
...
def build_late_fusion():
    ...
    x = Reshape((1, K.int_shape(x)[1]))(x)
    y = Reshape((1, K.int_shape(y)[1]))(y)

    w = NoParaSelfAttentionLayer()([x, y])
    print(K.int_shape(w))

    w = Reshape((K.int_shape(w)[1]*K.int_shape(w)[2],))(w)
    print(K.int_shape(w))

    w = Dense(len(activity_list), activation='softmax')(w)
    print(K.int_shape(w))

    model = Model(inputs=[input_t, input_w], outputs=w)
    model.summary()
    return model
...
def build_mid_fusion():
    ...
    z = Reshape((K.int_shape(w)[1]*K.int_shape(w)[2],))(w)
    print(K.int_shape(z))
...
def _run_(_train_features, _train_labels, _test_features, _test_labels):
    ...
    model.fit([_train_features_t, _train_features_w], _train_labels, verbose=1, batch_size=64, epochs=20, shuffle=True)
    ...
    results = 'act_acw' + ',' + str(fusion) + ',' + str(accuracy)+',' + str(f_score)
    print(results)
