...
def build_early_fusion():
    ...
    # input_wdcpm = concatenate([input_w, input_dc, input_pm], axis=2)
    input_wdcpm = EarlyAttentionLayer(feature_length+(dc_frames_per_second*dc_frame_size)+(pm_frames_per_second*pm_frame_size))([input_w, input_dc, input_pm])

    x = TimeDistributed(Conv1D(32, kernel_size=5, activation='relu'))(input_wdcpm)
    ...
    model = Model(inputs=[input_w, input_dc, input_pm], outputs=x)
    model.summary()
    return model
...
def _run_(acw_train_features, dc_train_features, pm_train_features, train_labels, acw_test_features, dc_test_features,
    ...
    results = 'acw_dc_pm' + ',' + str(fusion)+',' + str(accuracy)+',' + str(f_score)
    print(results)
