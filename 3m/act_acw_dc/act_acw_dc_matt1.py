...
def build_early_fusion():
    ...
    z = Reshape((window, feature_length+feature_length+(dc_frames_per_second*dc_frame_size)))(input_twdc)
...
def build_mid_fusion():
    ...
    c = concatenate([x, y, z])
    score = Dense(100, activation='tanh')(c)
    attention_probs = Dense(3600, activation='softmax', name='attention_vec')(score)
    c = Multiply()([c, attention_probs])
...
def build_late_fusion():
    ...
    c = concatenate([x, y, z])
    score = Dense(100, activation='tanh')(c)
    attention_probs = Dense(300, activation='softmax', name='attention_vec')(score)
    c = Multiply()([c, attention_probs])
