...
results_file = 'act_acw_dc_att12.csv'
...
class AverageLayer(Layer):
    ...
    def call(self, inputs):
        # (, 200)
        z = inputs[0]+inputs[1]+inputs[2]+inputs[3]+inputs[4]+inputs[5]
...
def build_mid_fusion():
    ...
    xz = CrossSensorLayer(1200)([x, z])
    yx = CrossSensorLayer(1200)([y, x])
    yz = CrossSensorLayer(1200)([y, z])
    zx = CrossSensorLayer(1200)([z, x])
    zy = CrossSensorLayer(1200)([z, y])

    zz = AverageLayer()([xy, xz, yx, yz, zx, zy])

    a = AttentionLayer(3600)([x, y, z])
    c = concatenate([a, zz])
...
def build_late_fusion():
    ...
    xy = CrossSensorLayer(100)([x, y])
    xz = CrossSensorLayer(100)([x, z])
    yx = CrossSensorLayer(100)([y, x])
    yz = CrossSensorLayer(100)([y, z])
    zx = CrossSensorLayer(100)([z, x])
    zy = CrossSensorLayer(100)([z, y])
...
def _run_(act_train_features, acw_train_features, dc_train_features, train_labels, act_test_features, acw_test_features,
    ...
    results = 'act_acw_dc' + ',' + str(fusion)+',' + str(accuracy)+',' + str(f_score)
    print(results)
