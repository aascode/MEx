...
path = '/Volumes/1708903/MEx/Data/pm_scaled/min/'
results_file = 'pm_att.csv'
...
def _run_(_train_features, _train_labels, _test_features, _test_labels):
    ...
    _train_features = np.reshape(_train_features, (_train_features.shape[0], window * frames_per_second, 32*16))
    _train_features = np.expand_dims(_train_features, 4)
    print(_train_features.shape)

    _test_features = np.array(_test_features)
    print(_test_features.shape)
    _test_features = np.reshape(_test_features, (_test_features.shape[0], window * frames_per_second, 32*16))
    _test_features = np.expand_dims(_test_features, 4)
    print(_test_features.shape)

    model = build_1D_model()
    model.compile(optimizer='adadelta', loss='categorical_crossentropy', metrics=['accuracy'])
    model.fit(_train_features, _train_labels, verbose=1, batch_size=32, epochs=30, shuffle=True)
    ...
    results = 'pm,' + str(i)+',' + str(accuracy)+',' + str(f_score)
    print(results)
