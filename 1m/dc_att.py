...
import numpy as np
from keras.layers import Input, Dense, BatchNormalization, Conv2D, MaxPooling2D, Flatten
...
path = '/Volumes/1708903/MEx/Data/dc_scaled/min/'
results_file = 'dc_att.csv'
...
def read():
    ...
    for subject in subjects:
        ...
        activities = [f for f in os.listdir(subject_path) if not f.startswith('.')]
...
def _run_(_train_features, _train_labels, _test_features, _test_labels):
    ...
    results = 'dc,' + str(i)+',' + str(accuracy)+',' + str(f_score)
    print(results)
