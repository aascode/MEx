...
results_file = '/Volumes/1708903/MEx/results/s_f/pm.csv'
...
def _average(_data_):
    _data_ = np.array(_data_)
    _avg_ = np.average(_data_, axis=0)
    print(_avg_.shape)
    return _avg_.tolist()


def trim_average(_data):
    _length = len(_data)
    _inc = _length/(window*frames_per_second)
    _new_data = []
    for i in range(window*frames_per_second):
        if (i+1)*_inc >= _length:
            ...
            _end = (i+1)*_inc
        _new_data.append(_average(_data[i*_inc:_end]))
    return _new_data
    ...
    for subject in _data:
        ...
        for activity in activities:
            ...
            for item in activity_data:
                time_windows.append(trim_average(item))
